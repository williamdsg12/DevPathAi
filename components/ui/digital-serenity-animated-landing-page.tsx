...
interface DigitalSerenityProps {
  /**
   * The floating dots stay hidden until the first scroll event, matching the
   * original design. When this component is used as a full-viewport hero with
   * `overflow-hidden` there is nothing to scroll, so the dots would never
   * appear — set this to `true` to start them on mount instead.
   */
  animateFloatingOnMount?: boolean
}
...
const DigitalSerenity = ({
  ...
  }, [scrolled, animateFloatingOnMount])
